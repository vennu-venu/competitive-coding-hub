...
updateProfileApiRoute.post("/verify-and-get-user", async (req, res) => {
  ...
  await jwt.verify(token, "cch", async (error, decodedObj) => {
    if (error) {
      ...
      let userData = await dbObj.collection("users").findOne({email: {$eq: email}});
      delete userData.password;
      res.send({message: "Succesful Request", verification: true, user: userData});
    }
  });
});
...
updateProfileApiRoute.post("/change", async (req, res) => {
  ...
  delete data.email;
  let dbObj = req.app.locals.databaseObj;
  await dbObj.collection("users").updateOne({email: {$eq: email}}, {$set: data});
